...
public class DPoPJwtServerAuthenticationConverter implements ServerAuthenticationConverter {
    ...
    public Mono<Authentication> convert(ServerWebExchange exchange) {
        return Mono.fromCallable(() ->
                        resolveFromAuthorizationHeaders(exchange.getRequest().getHeaders()))
                .map(tokens->{
                    var method=exchange.getRequest().getMethod();
                    var uri=exchange.getRequest().getURI();
                    return new DPopAuthenticationToken(method,uri,tokens);
                });
    }
